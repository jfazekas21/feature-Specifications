...
| CRC width | 16-bit | Change detection only — see §9 |
| Max CRC entries per file | 64 | Fixed-width manifest row — see §5 |
...
- Malicious-modification / tamper protection — the 16-bit CRC is change detection only, not authentication (see §9).
- Application-level reassembly of oversized single responses.
- Transports other than HTTP in v1 — IoT-hub and WebSocket sync are anticipated but unspecified (see §10.4).

---

## 3. Architecture

### 3.1 The manifest is the system
...
- **Chunk CRCs** — a 16-bit CRC for each chunk of the file (chunk size derived from file size, see §5).
- **File CRC** — a rollup over the file's content, used as the primary "did this file change" signal.

The device maintains its own cached CRC tree for every file it holds, persisted across power cycles (RAM or a local `.db`). A factory-fresh or cache-corrupted device rebuilds the tree by scanning every file — see the cold-start gate in §9.
...
## 5. Chunk Sizing — Fixed-Width Vector
...
5. Deleted files are removed locally. For each new/changed file, the device performs the per-file update (§6.2).
6. After each file update, the device recomputes its file CRC and confirms it matches the manifest.

### 6.2 Per-file update — size-first / optimistic append
...
When the manifest itself has changed extensively, the device may re-fetch it whole rather than range-fetching scattered changes (see §8).

---

## 8. Manifest-Format Caveat (`.db` does not append cleanly)
...
Decide this consciously rather than discovering it in testing. (Open item, §10.)

---

## 9. Where the Design Can Fail
...
1. **Manifest format** — SQLite `.db` (random-access friendly, but pages scatter on write) vs. an append-only/log-structured format (syncs as a tail-append, but needs a query strategy on-device). See §8.
2. **Chunk-leaf CRC width** — 16-bit keeps a maxed file's vector at 128 bytes (capped); the collision risk in §9 may argue for wider leaves at a storage cost. Confirm 16-bit leaves are acceptable given CRC-32 rollups.
3. **Tier boundaries** — the §5 thresholds (8 KB / 512 KB / 3 MB; optional 16 KB tier) are tunable. Lock the values and assign a format version before firmware integration.
...
## 11. Appendix — Required Firmware Measurement (Design Gate)
...
The measured number feeds directly into the chunk-leaf width decision (§10.2) and the cold-start risk (§9).

---

## Appendix B — Reference Figures

- 3 MB ÷ 8 KB = **384** chunks (binary units). At 2 bytes each, 768 bytes for a full 8 KB-chunk vector — which is why the fixed 64-entry cap (§5) exists.
- 3 MB ÷ 64 KB = **48** chunks — the large-file tier, comfortably under the 64 cap.
- 16-bit CRC value space = **65,536** → ~1-in-65,536 per-chunk collision probability (§9).
